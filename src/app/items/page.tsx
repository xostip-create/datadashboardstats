...
import type { Item, StockLevel, Staff } from '@/lib/data';
...
import { collection, doc, writeBatch, updateDoc, deleteDoc, getDocs, query, where, addDoc } from 'firebase/firestore';
...
function StaffManagementTab() {
    ...
    React.useEffect(() => {
        if (editingStaff) {
            form.setValue('name', editingStaff.name);
            setModalOpen(true);
        } else {
            form.reset({ name: '' });
        }
                    ...
                    <DialogContent className="sm:max-w-[425px]">
                        ...
                        <Form {...form}>
                            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
                                ...
                                />
                                <DialogFooter>
                                    <Button type="submit">{editingStaff ? 'Save Changes' : 'Save Staff'}</Button>
                                </DialogFooter>
                            </form>
                        </Form>
                    </DialogContent>
                                        ...
                                        <DropdownMenuContent align="end">
                                            ...
                                            <AlertDialog>
                                                ...
                                                <AlertDialogContent>
                                                    ...
                                                    <AlertDialogFooter>
                                                        <AlertDialogCancel>Cancel</AlertDialogCancel>
                                                        <AlertDialogAction onClick={() => handleDeleteStaff(s.id)} variant="destructive">Delete</AlertDialogAction>
                                                    </AlertDialogFooter>
                                                </AlertDialogContent>
                                            </AlertDialog>
                                        </DropdownMenuContent>
